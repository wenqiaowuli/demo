...
await sleep(300);
{
    ...
    ok(await designProbe('scopeCanvas', 480, 96, 70, 288, 288, GREEN), '荧光屏出现荧光绿波形');
...
{
    const combos = [
        { name: '直流×正弦 → 屏上竖直直线', x: 'dc', y: 'sine', region: [214, 70, 52, 288] },
        { name: '直流×直流(+40V) → 屏上静止亮点', x: 'dc', y: 'dc', region: [228, 164, 24, 24], ySlider: 40 },
        { name: '方波×方波 → 屏上矩形轨迹', x: 'square', y: 'square', region: [160, 134, 160, 160], xSlider: 60, ySlider: 60 },
        { name: '正弦×正弦 → 屏上李萨如椭圆', x: 'sine', y: 'sine', region: [160, 134, 160, 160] },
        { name: '余弦×余弦 → 屏上李萨如椭圆', x: 'cosine', y: 'cosine', region: [160, 134, 160, 160] },
        { name: '锯齿×余弦 → 屏上余弦波形', x: 'saw', y: 'cosine', region: [96, 70, 288, 288] },
    ];
...
console.log('== 7. 直流电压可调（−100~+100 V）与清屏');
{
    ...
    await click('btnStart');
    await sleep(1200);
    ok(await designProbe('scopeCanvas', 480, 174, 70, 16, 288, GREEN), '直流−60V×正弦 → 屏上水平直线（X=−2.4cm）');
